...
### Reports, Settlement, and Exports

Store reports aggregate booking, payment, deposit, asset, and staff activity by date range with optional branch filters. Reports avoid renter PII and raw location fields; exports are filtered snapshots with requester, report type, format, row count, and generated content metadata.

Settlements store period, store/branch, gross/online/cash/refund/penalty amounts, platform fee, payment fee, transfer payable, and state. Cash is included in reports but excluded from transfer payable unless the accepted settlement policy explicitly opts in. Commission plan and settlement cycle remain configurable policy inputs.
...
- `settlements`: query by `tenant_id`, `store_id`, `status`, `period_from`, `period_to`
- `report_exports`: query by `tenant_id`, `requested_by_user_id`, `type`, `created_at`
