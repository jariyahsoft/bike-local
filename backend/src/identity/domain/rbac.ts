import type { BranchId, StoreId, TenantId, UserId } from "../../shared/domain/index.js";
...
export const PERMISSION_CATALOG: Record<PermissionName, PermissionDefinition> = {
  "store.read": {
    ...
    description: "Read store profile and operational data within assigned scope.",
  },
  "store.update": {
    name: "store.update",
    resource: "store",
    ...
    description: "Update store details, settings, and approval-sensitive fields.",
  },
  "branch.create": {
    name: "branch.create",
    resource: "branch",
  ...
  "sos.location.read": {
    ...
    action: "read",
    scopes: ["store", "branch", "platform"],
    description: "Read renter location during an SOS or active ride support flow.",
  },
  "content.approve": {
    ...
    action: "approve",
    scopes: ["platform"],
    description: "Approve or reject routes, places, and review moderation actions.",
  },
  "platform.store.suspend": {
...
export const ROLE_PERMISSION_MATRIX: Record<RoleName, readonly PermissionGrant[]> = {
  RENTER: [
    { permission: "booking.read", scope: "user" },
  ],
  STORE_OWNER: [
...
const scopeMatches = (
  ...
  if (grant.scope === "user") {
    return actorUserId !== undefined && target.ownerUserId !== undefined && actorUserId === target.ownerUserId;
  }

  if (grant.scope === "tenant") {
    return assignment.tenantId !== undefined && target.tenantId !== undefined && assignment.tenantId === target.tenantId;
  }
  ...
  if (grant.scope === "branch") {
    return target.branchId !== undefined && assignment.branchIds?.includes(target.branchId) === true;
  }

  return false;
};

const collectGrantsForAssignment = (assignment: RoleAssignment): readonly PermissionGrant[] => {
  const base = ROLE_PERMISSION_MATRIX[assignment.role];
  const extras = assignment.grantedPermissions?.map((permission) => ({
    permission,
...
export const evaluatePermission = ({
  ...
  for (const assignment of assignments) {
    ...
    const grants = collectGrantsForAssignment(assignment).filter((grant) => grant.permission === permission);
    if (grants.length === 0) {
      continue;
    }
